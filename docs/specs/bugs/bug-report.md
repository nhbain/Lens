...
8. The input field for file patterns extends into the "Add" button element. This needs to be fixed. See the attached image. 
![alt text](image.png)
9. The state of items is not being updated in the document tree after saving changes to the source file. You have to refresh the document tree to see the changes. We need to ensure item state is correctly updated across both the document tree and editor after changed are made. 